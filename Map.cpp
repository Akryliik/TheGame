...
Map::Map(int size , int nbr_obs, int nbr_mon){
	...
	map_case[size/2][size/2].case_type = "Spawn"; // type de case player =pos du joueur
	
		
}

void Map::repartition (int nbr_pot_mana , int nbr_pot_heal , int nbr_obstacle,int nbr_monstre){
	std::random_device rd;  
	std::default_random_engine gen(rd());
	//std::mt19937 gen(rd()); 
	std::uniform_int_distribution<int> dis(1, map_size-1);//(rand() %100+1)-(100 - (map_size-1)
	int coord_x = dis(gen);
	int coord_y = dis(gen);
	for (int i = 0 ; i < nbr_pot_mana ; i++){		// placement random des potion mana
		while (map_case[coord_x][coord_y].case_type != "vide" ){
			coord_x = dis(gen);
			coord_y = dis(gen);
		}
		map_case[coord_x][coord_y].case_type = "mana" ;
	}
	for (int i = 0 ; i < nbr_pot_heal ; i++){		// placement random des potion de heal
		while (map_case[coord_x][coord_y].case_type != "vide" ){
			coord_x = dis(gen);
			coord_y = dis(gen);
		}
		map_case[coord_x][coord_y].case_type = "heal" ;
	}
	string diff_obstacle = {"Arbre" , "Rocher" };
	for (int k = 0 ; k < nbr_obstacle ; k++){			// placement random obstacle
		while (map_case[coord_x][coord_y].case_type != "vide"  ){
			coord_x = dis(gen);
			coord_y = dis(gen);
		}
	...
	for (int k = 0 ; k < nbr_monster ; k++){			// placement random monstre
		while (map_case[coord_x][coord_y].case_type != "vide"  ){
			coord_x = dis(gen);
			coord_y = dis(gen);
		}
		
		map_case[coord_x][coord_y].case_type = "monster";
	}// à faire random les monstre 
...
void Map::display(){
	for (int i = 0 ; i < map_size ; i++){
		for (int j = 0 ; j < map_size ; j++){
				cout << map_case[i][j].case_type <<" " <<endl;
		}
	}
}
